import golap from "../../../public/assets/golapgram.jpg";
import lotkon from "../../../public/assets/lotkon.jpg";
import mababa from "../../../public/assets/mababa.jpg";

import robi from "../../../public/assets/robi.jpg";
import pita from "../../../public/assets/pita.jpg";
import veg from "../../../public/assets/veg.jpg";

import banana from "../../../public/assets/banana.jpg";
import papaya from "../../../public/assets/papaya.jpg";
import malta from "../../../public/assets/malta.jpg";
import pomegranate from "../../../public/assets/pomegranate.jpg";
import watermelon from "../../../public/assets/watermelon.jpg";
import strawberries from "../../../public/assets/strawberries.jpg"

//events
import bullock from "../../../public/events/Bullock-Cart-Race_1.jpg"
import bullock2 from "../../../public/events/Bullock-Cart-Race_2.jpg"
import bullock3 from "../../../public/events/Bullock-Cart-Race_3.jpg"
import fishing from "../../../public/events/Fishing-Festival_1.jpg"
import fishing2 from "../../../public/events/Fishing-Festival_2.jpg"
import fishing3 from "../../../public/events/Fishing-Festival_3.jpg"
import fishing4 from "../../../public/events/Fishing-Festival_4.jpg"
import madhumashi from "../../../public/events/Madhumashi-Festival_1.jpg"
import madhumashi2 from "../../../public/events/Madhumashi-Festival_2.jpg"
import madhumashi3 from "../../../public/events/Madhumashi-Festival_3.jpg"  
import madhumashi4 from "../../../public/events/Madhumashi-Festival_4.jpg"  
import madhumashi5 from "../../../public/events/Madhumashi-Festival_5.jpg"  
import mustard from "../../../public/events/Mustard-Fair_1.jpg"
import mustard2 from "../../../public/events/Mustard-Fair_2.jpg"
import mustard3 from "../../../public/events/Mustard-Fair_3.jpg"
import nabanna from "../../../public/events/Nabanna-Festival_1.jpg"
import nabanna2 from "../../../public/events/Nabanna-Festival_2.jpg"
import nabanna3 from "../../../public/events/Nabanna-Festival_3.jpg"
import nabanna4 from "../../../public/events/Nabanna-Festival_1..jpg"
import nabanna5 from "../../../public/events/Nabanna.jpg"
import pitha2 from "../../../public/events/Pitha-Festival_2.jpg"
import pitha3 from "../../../public/events/Pitha-Festival_3.jpg"

//products
import alu from "../../../public/products/AluLoose.png"
import cabbage from "../../../public/products/Cabbage.png"
import cauliflower from "../../../public/products/Cauliflower.png"
import fishbox from "../../../public/products/Fresh-Fish-Mixed-Box.png"
import fishbox1 from "../../../public/products/Fresh-Fish-Mixed-Box-1.png"
import fishbox2 from "../../../public/products/Fresh-Fish-Mixed-Box-2.png"
import katla from "../../../public/products/Katla-Medium.png"
import rui from "../../../public/products/Rui-Deshi.png"
import sarputi from "../../../public/products/Sarputi.png"
import tomato from "../../../public/products/Tomato.png"
import vegetable from "../../../public/products/Seasonal-Mixed-Vegetable-Box1.png"
import vegetable1 from "../../../public/products/Seasonal-Mixed-Vegetable-Box2.png"
import capsicum from "../../../public/products/Mixed-Capsicum.png"

//tourism spots
import golap1 from "../../../public/projects/RoseVillage_1.jpg"
import golap2 from "../../../public/projects/RoseVillage_2.jpg"
import golap3 from "../../../public/projects/RoseVillage_3.jpg"
import golap4 from "../../../public/projects/RoseVillage_4.jpg"
import golap5 from "../../../public/projects/RoseVillage_5.jpg"
import golap6 from "../../../public/projects/RoseVillage_6.jpg"

import lotkon1 from "../../../public/projects/LotkonBari_1.jpg"
import lotkon2 from "../../../public/projects/LotkonBari_2.jpg"
import lotkon3 from "../../../public/projects/LotkonBari_3.jpg"
import lotkon4 from "../../../public/projects/LotkonBari_4.jpg"
import lotkon5 from "../../../public/projects/LotkonBari_5.jpg"
import lotkon6 from "../../../public/projects/LotkonBari_6.jpg"
import lotkon7 from "../../../public/projects/LotkonBari_7.jpg"
import lotkon8 from "../../../public/projects/LotkonBari_8.jpg"
import lotkon9 from "../../../public/projects/LotkonBari_9.jpg"

import shimul1 from "../../../public/projects/ShimulBagan_1.jpg"
import shimul2 from "../../../public/projects/ShimulBagan_2.jpg"
import shimul3 from "../../../public/projects/ShimulBagan_3.jpg"
import shimul4 from "../../../public/projects/ShimulBagan_4.jpg"
import shimul5 from "../../../public/projects/ShimulBagan_5.jpg"
import shimul6 from "../../../public/projects/ShimulBagan_6.jpg"
import shimul7 from "../../../public/projects/ShimulBagan_7.jpg"
import shimul8 from "../../../public/projects/ShimulBagan_8.jpg"
import shimul9 from "../../../public/projects/ShimulBagan_9.jpg"

import mangohub1 from "../../../public/projects/MangoGarden_1.jpg"
import mangohub2 from "../../../public/projects/MangoGarden_2.jpg"
import mangohub3 from "../../../public/projects/MangoGarden_3.jpg"
import mangohub4 from "../../../public/projects/MangoGarden_4.jpg"


import teagardens1 from "../../../public/projects/TeaGardensEcoTour_1.jpg"
import teagardens2 from "../../../public/projects/TeaGardensEco-Tour_2.jpg"
import teagardens3 from "../../../public/projects/TeaGardensEco-Tour_3.jpg"
import teagardens4 from "../../../public/projects/TeaGardensEco-Tour_4.jpg"
import teagardens5 from "../../../public/projects/TeaGardensEco-Tour_5.jpg"
import teagardens6 from "../../../public/projects/TeaGardensEco-Tour_6.jpg"
import teagardens7 from "../../../public/projects/TeaGardensEco-Tour_7.jpg"
import teagardens8 from "../../../public/projects/TeaGardensEco-Tour_8.jpg"
import teagardens9 from "../../../public/projects/TeaGardensEco-Tour_9.jpg"
import teagardens10 from "../../../public/projects/TeaGardensEco-Tour_10.jpg"

//mababa
import manbaba1 from "../../../public/projects/Ma-BabaAgroFarmLtd._1.jpg"
import manbaba2 from "../../../public/projects/Ma-BabaAgroFarmLtd._2.jpg"
import manbaba3 from "../../../public/projects/Ma-BabaAgroFarmLtd._3.jpg"
import manbaba4 from "../../../public/projects/Ma-BabaAgroFarmLtd._4.jpg"
import manbaba5 from "../../../public/projects/Ma-BabaAgroFarmLtd._5.jpg"
import manbaba6 from "../../../public/projects/Ma-BabaAgroFarmLtd._6.jpg"
import manbaba7 from "../../../public/projects/Ma-BabaAgroFarmLtd._7.jpg"
import manbaba8 from "../../../public/projects/Ma-BabaAgroFarmLtd._8.jpg"
import manbaba9 from "../../../public/projects/Ma-BabaAgroFarmLtd._9.jpg"


const projects = [
  {
    id: 5,
    photo: mababa ,
    gallery: [manbaba1, manbaba2, manbaba3, manbaba4, manbaba5, manbaba6, manbaba7, manbaba8, manbaba9],
    name: "Ma-Baba Agro Farm Ltd.",
    season: "Year-Round",
    location: "Sherpur Sadar, Mymensingh",
    time: "7:00 AM - 7:00 PM",
    entryFee: "100 BDT",
    facilities: ["Accommodation", "Restaurant", "Parking"],
    details: {
      description: `Explore the grandeur of an 800-acre integrated farm producing a variety of fruits, vegetables, and livestock.`,
      visitorFacilities: [
        "Transport Service: Rent-a-car options.",
        "Comfort & Hospitality: Clean restrooms and cottages.",
        "Farm Produce Shopping: Buy fresh products at affordable prices."
      ],
      highlights: ["Learn modern farming techniques", "Ideal for family outings"]
    }
  },
  {
    id: 6,
    photo: teagardens1,
    gallery: [teagardens1, teagardens2, teagardens3, teagardens4, teagardens5, teagardens6, teagardens7, teagardens8, teagardens9, teagardens10],
    name: "Tea Gardens Eco-Tour",
    season: "Year-Round",
    location: "Sreemangal, Sylhet",
    time: "7:00 AM - 6:00 PM",
    entryFee: "300 BDT",
    facilities: ["Guided Tours", "Eco-Stay Options", "Tea-Tasting Sessions"],
    details: {
      description: `Embark on a refreshing journey to the lush tea gardens of Sreemangal.`,
      visitorFacilities: [
        "Rent-a-car or public transport assistance.",
        "On-site food options with Sylheti flavors.",
        "Farm-to-table shopping for organic tea leaves."
      ],
      highlights: ["Guided tours through plantations", "Eco-friendly accommodations"]
    }
  },
  {
    id: 1,
    photo: golap,
    gallery: [golap1, golap2, golap3, golap4, golap5, golap6],
    name: "গোলাপ গ্রাম (Rose Village)",
    season: "November - February",
    location: "Sahdullahpur, Savar",
    time: "8:00 AM - 6:00 PM",
    entryFee: "100 BDT",
    facilities: ["Parking", "Restaurant", "Photoshoot Spots"],
    details: {
      description: `Imagine an entire village turned into a rose garden! The highlands are adorned with vibrant Mirandi roses in shades of red, yellow, and white.`,
      visitorFacilities: [
        "Transport Service: Convenient rent-a-car options.",
        "Comfort & Hospitality: Reception, restrooms, and cozy cottage accommodations.",
        "Food & Refreshments: Safe drinking water and delicious meals.",
        "Affordable Farm Produce: Buy fresh agricultural products.",
        "Educational & Recreational Opportunities: Ideal for study tours or picnics."
      ],
      audience: ["Corporate Teams", "Schools & Colleges", "Nature Enthusiasts"]
    }
  },
  {
    id: 2,
    photo: lotkon,
    gallery: [lotkon1, lotkon2, lotkon3, lotkon4, lotkon5, lotkon6, lotkon7, lotkon8, lotkon9],
    name: "লটকন বাগান (Lotkon Bari)",
    season: "May - July",
    location: "Shibpur, Narsingdi",
    time: "9:00 AM - 5:00 PM",
    entryFee: "400 BDT",
    facilities: ["Parking", "Toilets", "Cafeteria"],
    details: {
      description: `Experience the charm of Lotkon Bari, famous for producing the finest lotkon!`,
      visitorFacilities: [
        "Transport Service: Rent-a-car options for easy travel.",
        "Comfortable Amenities: Reception area, restrooms, and cottages.",
        "Food & Drinks: Safe drinking water and local meals.",
        "Affordable Farm Produce: Fresh fruits at discounted rates."
      ],
      highlights: ["Unlimited lotkon tasting", "Special discounted rates for purchases"]
    }
  },
  {
    id: 3,
    photo: shimul1,
    gallery: [shimul1, shimul2, shimul3, shimul4, shimul5, shimul6, shimul7, shimul8, shimul9],
    name: "Shimul Bagan (Silk Cotton Forest)",
    season: "February - March",
    location: "Sunamganj",
    time: "9:00 AM - 6:00 PM",
    entryFee: "300 BDT",
    facilities: ["Parking", "Walking Trails", "Picnic Spots"],
    details: {
      description: `Explore the vibrant red silk cotton flowers transforming the forest into a mesmerizing paradise.`,
      visitorFacilities: [
        "Secure parking spaces.",
        "Well-maintained walking trails.",
        "Local snacks and refreshments."
      ],
      highlights: ["Nature’s Wonderland", "Perfect for photography", "Ideal picnic destination"]
    }
  },
  {
    id: 4,
    photo: mangohub1,
    gallery: [mangohub1, mangohub2, mangohub3, mangohub4],
    name: "Mango Tourism Hub",
    season: "May - July",
    location: "Chapai Nawabganj",
    time: "8:00 AM - 5:00 PM",
    entryFee: "100 BDT",
    facilities: ["Seasonal Fruit Picking", "Guided Farm Tours", "On-Site Farm Store"],
    details: {
      description: `Indulge in unlimited mango tasting straight from the orchards in the Mango Capital.`,
      visitorFacilities: [
        "Seasonal Fruit Picking: Pick and eat fresh mangoes.",
        "On-Site Farm Store: Purchase mangoes and organic products.",
        "Transport Assistance: Rent-a-car options for easy travel."
      ],
      highlights: ["Unlimited Mango Eating", "Learn mango cultivation"]
    }
  },
  
];





const projectBenefits = [
  {
    id: 1,
    benefit: "যাতায়াতের জন্য নিজস্ব রেন্ট-এ কার সুবিধা",
  },
  {
    id: 2,
    benefit: "অভ্যর্থনা,রেস্টরুম ও কটেজ সার্ভিস",
  },
  {
    id: 3,
    benefit: "সুপেয় পানি ও খাবারের সুব্যবস্থা",
  },
  {
    id: 4,
    benefit: "কৃষিখামার থেকে সুলভ মূল্যে কৃষিপণ্য ক্রয়ের সুবিধা",
  },
  {
    id: 5,
    benefit: "শিক্ষাভ্রমণ বা পিকনিকের সুব্যবস্থা",
  },
]


const events = [
    {
      id: 1,
      name: "রবি উৎসব (Robi Festival)",
      location: "Ashulia, Savar",
      date: "January 15-20, 2025",
      time: "9:00 AM - 8:00 PM",
      description: "The festival will showcase a wide variety of winter vegetables and agricultural products produced by marginal farmers. Additionally, there will be a collection of household items and seasonal fruits.",
      facilities: ["Parking", "Toilets", "Food Corner"],
      isCompleted: false,
      photo:robi,
      gallery:[robi,veg]
    },
    {
      id: 2,
      name: "পিঠা উৎসব (Pitha Festival)",
      location: "Jagannath University",
      date: "January 28, 2025",
      time: "10:00 AM - 5:00 PM",
      description: "Under the theme 'New grains, new spirit; let’s indulge in the aroma of Pitha,' Jagannath University will host the Pitha Festival 2025 at its new academic building. Organized by students, the festival will showcase traditional pithas (rice cakes) and desserts from across the country.",
      facilities: ["Parking", "Restrooms", "Seating arrangements", "Free entry"],
      isCompleted: false,
      photo:pitha2,
      gallery:[pitha2,pitha3,pita]
    },
    {
      id: 3,
      name: "মধুমাসী উৎসব (Madhumashi Festival)",
      location: "Sarighat, Keraniganj",
      date: "May 15-29, 2025",
      time: "8:00 AM - 9:00 PM",
      contact: "01914-789012",
      description: "Experience a delightful summer fruit festival featuring organic fruits from 64 districts across Bangladesh, including Rajshahi, Chapainawabganj, Dinajpur, and Barisal.",
      facilities: ["Parking", "Toilets", "Cafeteria"],
      isCompleted: false,
      photo:madhumashi,
      gallery:[madhumashi,madhumashi2,madhumashi3,madhumashi4,madhumashi5]
    },
    {
      id: 4,
      name: "গরুর গাড়ির দৌড় প্রতিযোগিতা (Bullock Cart Race)",
      location: "Raiganj, Sirajganj",
      date: "12-13 Chaitra, 1431 (Bengali Calendar)",
      time: "9:00 AM - 1:00 PM",
      description: "Experience the charm of rural Bengal with the traditional bullock cart race, featuring participants from various regions across the country. Alongside the thrilling race, enjoy a vibrant fair showcasing local handicrafts and delicious regional foods.",
      facilities: ["Parking", "Toilets", "Seating arrangements"],
      isCompleted: false,
      photo:bullock,
      gallery:[bullock,bullock2,bullock3]
    },
    {
      id: 5,
      name: "সরিষা মেলা (Mustard Fair)",
      location: "Ghatail, Tangail",
      date: "20–25 Magh, 1431 (Bengali Calendar)",
      time: "9:00 AM – 6:00 PM",
      description: "Experience the beauty of mustard fields and witness the traditional process of mustard oil production at this vibrant fair. Visitors can purchase mustard oil, honey, and a variety of other local products.",
      facilities: ["Toilets", "Food corners", "Traditional rural games"],
      isCompleted: false,
      photo:mustard,
      gallery:[mustard,mustard2,mustard3]
    },
    {
      id: 6,
      name: "মাছ ধরার উৎসব (Fishing Festival)",
      location: "Kishoreganj",
      date: "October 2024",
      time: "8:00 AM – 4:00 PM",
      description: "Join the Fishing Festival, a celebration of rural tradition and experience the thrill of fishing in the picturesque haor region alongside locals. From the early morning light, thousands will gather with traditional fishing tools such as bamboo-made polo, nets, and triangular push nets.",
      facilities: ["Boat rentals", "Food stalls", "Security personnel"],
      isCompleted: false,
      photo:fishing,
      gallery:[fishing,fishing2,fishing3,fishing4]
    },
    {
      id: 7,
      name: "নবান্ন উৎসব (Nabanna Festival)",
      location: "Chaitanyapur, Godagari Upazila, Rajshahi",
      date: "November 17, 2024",
      time: "8:00 AM – 2:00 PM",
      description: "In the Barind region, farmers celebrate the Nabanna Festival on the first day of Agrahayan by harvesting rice and preparing traditional delicacies like rice pudding (kheer), payesh, and pitha. This event highlights the rich agricultural traditions of the area with a feast of rural heritage foods. The festival in Chaitanyapur, organized by National Award-winning farmer Md. Moniruzzaman Monir, features indigenous women participating in rice harvesting to honor the agricultural and cultural legacy of the Barind region.",
      facilities: ["Parking", "Toilets", "Food stalls"],
      isCompleted: true,
      photo:nabanna,
      gallery:[nabanna,nabanna2,nabanna3,nabanna4,nabanna5]
    }
]

const eventBenefits = [
  {
    id: 1,
    benefit: "যাতায়াতের জন্য রেন্ট-এ কার সুবিধা",
  },
  {
    id: 2,
    benefit: "অভ্যর্থনা,রেস্টরুম  সার্ভিস",
  },
  {
    id: 3,
    benefit: "সুপেয় পানি ও খাবারের সুব্যবস্থা",
  },
  {
    id: 4,
    benefit: "সুলভ মূল্যে কৃষিপণ্য ক্রয়ের সুবিধা",
  },
  {
    id: 5,
    benefit: "নিরাপত্তার নিশ্চয়তা",
  },
]

const products = [
  {
    id: 1,
    name: "Paka Pepe (Ripen Papaya)",
    category: "Fruits",
    price: 500,
    quantity: "5 Piece",
    deliveryTime: "Delivery 1-2 days",
    farm: "Gazipur",
    owner: "Dulal Miya",
    photo: papaya,
    isFeatured: true,
  },
  {
    id: 2,
    name: "Paka Pepe (Ripen Papaya)",
    category: "Fruits",
    price: 950,
    quantity: "10 Piece",
    deliveryTime: "Delivery 1-2 days",
    farm: "Gazipur",
    owner: "Dulal Miya",
    photo: papaya,
    isFeatured: true,
  },
  {
    id: 3,
    name: "Paka Pepe (Ripen Papaya)",
    category: "Fruits",
    price: 1400,
    quantity: "15 Piece",
    deliveryTime: "Delivery 1-2 days",
    farm: "Gazipur",
    owner: "Dulal Miya",
    photo: papaya,
    isFeatured: true,
  },
  {
    id: 4,
    name: "Narikel (Coconut)",
    category: "Fruits",
    price: 700,
    quantity: "5 Piece",
    deliveryTime: "Delivery 1-2 days",
    farm: "Noakhali",
    owner: "Delwar Hossain Riyad",
    photo: watermelon,
    isFeatured: false,
  },
  {
    id: 5,
    name: "Narikel (Coconut)",
    category: "Fruits",
    price: 1350,
    quantity: "10 Piece",
    deliveryTime: "Delivery 1-2 days",
    farm: "Noakhali",
    owner: "Delwar Hossain Riyad",
    photo: watermelon,
    isFeatured: false,
  },
  {
    id: 6,
    name: "Narikel (Coconut)",
    category: "Fruits",
    price: 2000,
    quantity: "15 Piece",
    deliveryTime: "Delivery 1-2 days",
    farm: "Noakhali",
    owner: "Delwar Hossain Riyad",
    photo: watermelon,
    isFeatured: false,
  },
  {
    id: 7,
    name: "Guava",
    category: "Fruits",
    price: 500,
    quantity: "5 Kg",
    deliveryTime: "Delivery 1-2 days",
    farm: "Noakhali",
    owner: "Manzurul Karim",
    photo: banana,
    isFeatured: false,
  },
  {
    id: 8,
    name: "Guava",
    category: "Fruits",
    price: 950,
    quantity: "10 Kg",
    deliveryTime: "Delivery 1-2 days",
    farm: "Noakhali",
    owner: "Manzurul Karim",
    photo: banana,
    isFeatured: false,
  },
  {
    id: 9,
    name: "Meher Shagor Kola (Banana )",
    category: "Fruits",
    price: 500,
    quantity: "50 pcs",
    deliveryTime: "Delivery 1-2 days",
    farm: "Narsingdi",
    owner: "Mahtab Karim",
    photo: banana,
    isFeatured: false,
  },
  {
    id: 10,
    name: "Meher Shagor Kola (Banana )",
    category: "Fruits",
    price: 950,
    quantity: "100 pcs",
    deliveryTime: "Delivery 1-2 days",
    farm: "Narsingdi",
    owner: "Mahtab Karim",
    photo: banana,
    isFeatured: true,
  },
  {
    id: 11,
    name: "Mango: Katimon",
    category: "Fruits",
    price: 2000,
    quantity: "5 KG",
    deliveryTime: "Delivery 1-2 days",
    farm: "Bagha, Rajshahi",
    owner: "KM Siddiqui",
    photo: malta,
    isFeatured: false,
  },
  {
    id: 12,
    name: "Mango: Katimon",
    category: "Fruits",
    price: 3900,
    quantity: "10 KG",
    deliveryTime: "Delivery 1-2 days",
    farm: "Bagha, Rajshahi",
    owner: "KM Siddiqui",
    photo: malta,
    isFeatured: false,
  },
  {
    id: 13,
    name: "Mango: Katimon",
    category: "Fruits",
    price: 7600,
    quantity: "20 KG",
    deliveryTime: "Delivery 1-2 days",
    farm: "Bagha, Rajshahi",
    owner: "KM Siddiqui",
    photo: malta,
    isFeatured: false,
  },
  {
    id: 14,
    name: "Seasonal Mixed Vegetable Box 1",
    category: "Vegetables",
    price: 1000,
    quantity: "9-10 kg",
    deliveryTime: "Delivery 1-2 days",
    farm: "Manikganj",
    owner: "Atikur Rahman",
    photo: vegetable,
    isFeatured: true,
  },
  {
    id: 15,
    name: "Seasonal Mixed Vegetable Box 2",
    category: "Vegetables",
    price: 1450,
    quantity: "10-12 kg",
    deliveryTime: "Delivery 1-2 days",
    farm: "Manikganj",
    owner: "Atikur Rahman",
    photo: vegetable1,
    isFeatured: true,
  },
  {
    id: 16,
    name: "Mixed Capsicum",
    category: "Vegetables",
    price: 2000,
    quantity: "3 Kg",
    deliveryTime: "Delivery 1-2 days",
    farm: "Manikganj",
    owner: "Atikur Rahman",
    photo: capsicum,
    isFeatured: true,
  },
  {
    id: 17,
    name: "Badhakopi (Cabbage) & Fulcopy (Cauliflower) Combo",
    category: "Vegetables",
    price: 800,
    quantity: "8 pieces",
    deliveryTime: "Delivery 1-2 days",
    farm: "Manikganj",
    owner: "Atikur Rahman",
    photo: cauliflower,
    isFeatured: false,
  },
  {
    id: 18,
    name: "Alu Loose (Cardinal)",
    category: "Vegetables",
    price: 800,
    quantity: "10 kg",
    deliveryTime: "Delivery 1-2 days",
    farm: "Narsingdi",
    owner: "Mahtab Karim",
    photo: alu,
    isFeatured: false,
  },
  {
    id: 19,
    name: "Alu Loose (Cardinal)",
    category: "Vegetables",
    price: 1550,
    quantity: "20 kg",
    deliveryTime: "Delivery 1-2 days",
    farm: "Narsingdi",
    owner: "Mahtab Karim",
    photo: alu,
    isFeatured: false,
  },
  {
    id: 20,
    name: "Tomato (Pesticide-Free)",
    category: "Vegetables",
    price: 1000,
    quantity: "5 kg",
    deliveryTime: "Delivery 1-2 days",
    farm: "Daudkandi, Cumilla",
    owner: "Uzzal Das",
    photo: tomato,
    isFeatured: false,
  },
  {
    id: 21,
    name: "Tomato (Pesticide-Free)",
    category: "Vegetables",
    price: 1900,
    quantity: "10 kg",
    deliveryTime: "Delivery 1-2 days",
    farm: "Daudkandi, Cumilla",
    owner: "Uzzal Das",
    photo: tomato,
    isFeatured: false,
  },
  {
    id: 22,
    name: "Fresh Fish Mixed Box (Rui, Catla, Mrigel, Silver Carp and Grass Carp",
    category: "Fish",
    price: 2200,
    quantity: "5 kg mix",
    deliveryTime: "Delivery 1-2 days",
    farm: "Paba, Rajshahi",
    owner: "Kamrul",
    photo: fishbox,
    isFeatured: true,
  },
  {
    id: 23,
    name: "Fresh Fish Mixed Box (Rui, Catla, Mrigel, Silver Carp and Grass Carp",
    category: "Fish",
    price: 3800,
    quantity: "10 kg mix",
    deliveryTime: "Delivery 1-2 days",
    farm: "Paba, Rajshahi",
    owner: "Kamrul",
    photo: fishbox2,
    isFeatured: true,
  },
  {
    id: 24,
    name: "Rui Deshi (P)",
    category: "Fish",
    price: 2500,
    quantity: "5 kg",
    deliveryTime: "Delivery 1-2 days",
    farm: "Paba, Rajshahi",
    owner: "Kamrul",
    photo: rui,
    isFeatured: true,
  },
  {
    id: 25,
    name: "Rui Deshi (P)",
    category: "Fish",
    price: 5000,
    quantity: "10 kg",
    deliveryTime: "Delivery 1-2 days",
    farm: "Paba, Rajshahi",
    owner: "Kamrul",
    photo: rui,
    isFeatured: true,
  },
  {
    id: 26,
    name: "Katla Medium",
    category: "Fish",
    price: 2000,
    quantity: "5 kg",
    deliveryTime: "Delivery 1-2 days",
    farm: "Paba, Rajshahi",
    owner: "Kamrul",
    photo: katla,
    isFeatured: false,
  },
  {
    id: 27,
    name: "Katla Medium ",
    category: "Fish",
    price: 4000,
    quantity: "10 kg",
    deliveryTime: "Delivery 1-2 days",
    farm: "Paba, Rajshahi",
    owner: "Kamrul",
    photo: katla,
    isFeatured: false,
  },
  {
    id: 28,
    name: "Sarputi ",
    category: "Fish",
    price: 1500,
    quantity: "5 kg",
    deliveryTime: "Delivery 1-2 days",
    farm: "Paba, Rajshahi",
    owner: "Kamrul",
    photo: sarputi,
    isFeatured: false,
  },
  {
    id: 29,
    name: "Sarputi",
    category: "Fish",
    price: 3000,
    quantity: "10 kg",
    deliveryTime: "Delivery 1-2 days",
    farm: "Paba, Rajshahi",
    owner: "Kamrul",
    photo: sarputi,
    isFeatured: false,
  },
];

const FAQ = [
  {
    id: 1,
    question: "Are there any discounts for group bookings?",
    answer: "Group consist of more than 5 numbers will get 20% discount."
  },
  {
    id: 2,
    question: "Is there parking available for visitors?",
    answer: "Yes,but it will charge 20 taka for bike & 50 taka for car."
  },
  {
    id: 3,
    question: "Are there guided tours available?",
    answer: "Currently not available but receptionist will help you by providing information and others."
  },
  {
    id: 4,
    question: "Can visitors bring their own food?",
    answer: "No,why you need to bring your food when you can get fresh organic and delicious food from our kitchen."
  },
  {
    id: 5,
    question: "Are visitors allowed to take photographs or record videos during the tour?",
    answer: "obviously.There you also get photography corner and on payment DSLR service."
  },

]


export { projects, products, events, eventBenefits, projectBenefits, FAQ };